...
def create_recipe():
    """Vytvoří nový recept s počtem porcí"""
    try:
        data = request.get_json()
        
        # Získání dat z requestu
        name = data.get('name')
        portions = data.get('portions', 1)  # Počet porcí, default 1
        ingredients = data.get('ingredients', [])
        user_id = data.get('user_id')
        profile_id = data.get('profile_id')
        
        # Výpočet nutričních hodnot pro jednu porci
        total_nutrition = calculate_total_nutrition(ingredients)
        nutrition_per_portion = calculate_nutrition_per_portion(total_nutrition, portions)
        ...
        return jsonify({'success': True, 'recipe_id': response.data[0]['id']})
        
    except Exception as e:
        return jsonify({'error': str(e)}), 500

def calculate_total_nutrition(ingredients):
    """Vypočítá celkové nutriční hodnoty ze všech ingrediencí"""
    total = {
        'calories': 0,
        'protein': 0,
        'carbs': 0,
    ...
    for ingredient in ingredients:
        ...
        nutrition = ingredient.get('nutritionalDetails', {})
        
        # Přepočet na základní jednotky
        if unit == 'kg':
            amount *= 1000
        elif unit == 'l':
            amount *= 1000
        
        factor = amount / 100  # Přepočet na 100g/ml
        
        # Přidání živin
        for key in total.keys():
            if key in nutrition:
                total[key] += (parse_nutrient_value(nutrition[key]) or 0) * factor
    
    return total

def calculate_nutrition_per_portion(total_nutrition, portions):
    """Vypočítá nutriční hodnoty pro jednu porci"""
    if portions <= 0:
        portions = 1
    
    per_portion = {}
    for key, value in total_nutrition.items():
        per_portion[key] = value / portions
    
    return per_portion

def parse_nutrient_value(value):
    """Parsuje nutriční hodnotu z řetězce na číslo"""
    if isinstance(value, (int, float)):
        return value
    if isinstance(value, str):
        # Odstranění jednotek a převod na číslo
